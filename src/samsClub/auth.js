  ...
  refresh: async () => {
    ...
    */
    ...
    try {
      ...
      await page.goto('https://www.samsclub.com/login?redirectURL=%2Fpharmacy%2Fimmunization%2Fform%3FimzType%3Dcovid');

      await page.fill('input[name=email]', process.env.SAMS_CLUB_EMAIL);
      await page.fill('input[name=current-password]', process.env.SAMS_CLUB_PASSWORD);
      await retry(async () => {
        await page.click('[type=submit]');
        await page.waitForSelector('input[aria-label="ZIP Code or city and state"]', {
          timeout: 5000,
        });
      }, {
        retries: 5,
     ...
     // await page.goto('https://www.samsclub.com/pharmacy/immunization/form?imzType=covid');
      ...
      await page.fill('input[aria-label="ZIP Code or city and state"]', '80620');
      await retry(async () => {
        await page.click('button[type=submit]');
        await page.waitForSelector('.sc-pharmacy-club-selection-club-radio-group input[type=radio]', {
          timeout: 5000,
        });
      }, {
        retries: 5,
      ...
      const ageResp = await page.waitForResponse('https://www.samsclub.com/api/node/vivaldi/v1/pharmacy/rules/ageEligibility');
      ...
      if (ageResp.status() === 412) {
        ...
        const foo = await findInFrames(page, '[aria-label="Human Challenge requires verification. Please press and hold the button until verified"]:visible');
        ...
        const ageResp2 = await page.waitForResponse('https://www.samsclub.com/api/node/vivaldi/v1/pharmacy/rules/ageEligibility');
      ...
      for (const cookie of await context.cookies()) {
        ...
        const putCookie = util.promisify(cookieJar.store.putCookie.bind(cookieJar.store));
        await putCookie(new Cookie({
          key: cookie.name,
          value: cookie.value,
          domain: cookie.domain.replace(/^\./, ''),
          path: cookie.path,
          expires: (cookie.expires && cookie.expires != -1) ? new Date(cookie.expires * 1000) : 'Infinity',

...
module.exports.findSamsClubStores = async () => {
  ...
  const resp = await got('https://www.samsclub.com/api/node/vivaldi/v2/clubfinder/list', {
    searchParams: {
  ...
  for (const store of resp.body) {
    ...
    if (store.address.state !== 'CO') {
      console.info(`  Skipping store in other state ${store.id}: ${store.address.state}`);
    } else {

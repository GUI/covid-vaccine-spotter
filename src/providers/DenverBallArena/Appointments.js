...
class Appointments {
  static async refreshStores() {
    ...
    const providerBrand = await ProviderBrand.query()
      .insert({
        provider_id: "denver_ball_arena",
        key: "denver_ball_arena",
    ...
    const resp = await retry(
      async () => {
        ...
        return got.post(
          "https://www.primarybio.com/test_groups/truecare24-cdphe/appointment_slot_availabilities",
          {
            headers: {
    ...
    await Store.query()
      .where("provider_id", "denver_ball_arena")

...
class Geocode {
  static async fix() {
    ...
    const stores = await Store.query().whereRaw("location IS NULL AND state IS NOT NULL AND city IS NOT NULL").orderBy("id");
    ...
    const geocodeResp = await got.post(
    ...
    for (const [storeId, bulkResult] of Object.entries(geocodeResp.body.results)) {
      const result = bulkResult?.response?.results?.[0];
      if (bulkResult?.query?.state !== result?.address_components?.state) {
        logger.warn(`States did not match for geocoding result: ${JSON.stringify(bulkResult)}`);
        continue;
      }

      await Store.query().findById(storeId).patch({
        location: `point(${result.location.lng} ${result.location.lat})`,
        location_source: 'geocodio',
      })
    }

    const geonamesStores = await Store.query().whereRaw("location IS NULL AND state IS NOT NULL AND city IS NOT NULL").orderBy("id");
    logger.info(`Geocoding ${geonamesStores.length} locations with Geonames...`);
    for (const [index, store] of geonamesStores.entries()) {
      logger.info(`Geocoding ${store.city}, ${store.state} #${store.id} (${index + 1} of ${geonamesStores.length})...`);
      ...
      if (result) {
        console.info(result);
        await Store.query().findById(store.id).patch({

exports.up = async function (knex) {
  // Suitable for 10 mile radius searches.
  await knex.raw(`
    CREATE MATERIALIZED VIEW country_grid_22km
    AS
    WITH
    country_boundary AS (
    ...
    country_grid AS (
      SELECT make_rect_grid(geom, 22000, 22000) AS geom
  ...
  `);
  await knex.raw(`
    CREATE MATERIALIZED VIEW state_grid_22km
    AS
    WITH
    country_boundary AS (
      SELECT st_union(boundaries::geometry) AS geom
  ...
  // Suitable for 5 mile radius searches.
  await knex.raw(`
    CREATE MATERIALIZED VIEW country_grid_11km
    AS
    WITH
    country_boundary AS (
      SELECT st_union(boundaries::geometry) AS geom
    ...
    country_grid AS (
      SELECT make_rect_grid(geom, 11000, 11000) AS geom
      FROM country_boundary
    ),
    country_grid_rows AS (
      SELECT (st_dump(geom)).geom
  ...
  `);
  await knex.raw(`
    CREATE MATERIALIZED VIEW state_grid_11km
    AS
    WITH
    country_boundary AS (
      SELECT st_union(boundaries::geometry) AS geom
  ...
  await knex.schema.table("country_grid_22km", (table) => {
    table.index("geom", null, "gist");
    table.index("centroid_location", null, "gist");
  });
  await knex.schema.table("state_grid_22km", (table) => {
    table.index("geom", null, "gist");
    table.index("centroid_location", null, "gist");
  });

  await knex.schema.table("country_grid_11km", (table) => {
    table.index("geom", null, "gist");
    table.index("centroid_location", null, "gist");
  });
  await knex.schema.table("state_grid_11km", (table) => {
    table.index("geom", null, "gist");
    table.index("centroid_location", null, "gist");
  });
};
